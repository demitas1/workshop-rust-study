...
`run(args: &[&str], ...)` の `&[&str]` を分解すると：

- `&str` — 文字列スライス（文字列データへの参照）
- `[&str]` — `&str` のスライス。サイズ不定なためそのままでは使えない
- `&[&str]` — スライスへの参照。ポインタと長さの2つの情報を持つ
...
| ヒープ確保 | する | しない |
| 所有権 | バッファを所有 | 借用のみ |
| 受け取れる型 | `Vec` のみ | `Vec` も配列も受け取れる |

関数の引数で受け取るだけなら `&[&str]` の方が軽量で柔軟。`&["a", "b"]` のようにリテラルで渡すと配列がスタックに作られ、それへの参照が渡される。

## 明示的にヒープで args を渡す

`vec!` マクロで `Vec<&str>` を作るとヒープにバッファが確保される。

```rust
#[test]
fn hello1() -> TestResult {
    let args: Vec<&str> = vec!["Hello there"];
    run(&args, "tests/expected/hello1.txt")
}
```

`&args` と書くと `Vec<&str>` が `&[&str]` として扱われる。これは Deref という仕組みによるもので、`Vec<T>` を参照すると自動的に `&[T]` に変換される。`run` の引数の型は `&[&str]` なので、`Vec` でも配列でも同じ関数に渡せる。
